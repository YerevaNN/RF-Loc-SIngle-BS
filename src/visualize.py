...
def visualize_pair(
    ...
    fig.add_trace(
        go.Scatter(
            x=[bs_location[0]], y=[bs_location[1]],
            mode='markers', marker=dict(size=5, color="blue"), name="BS location"
        )
    )
    fig.add_trace(
        go.Scatter(
            x=[ue_location[0]], y=[ue_location[1]],
    ...
    fig.add_trace(
        go.Scatter(
            x=[ue_location_pred_non_ml[0]], y=[ue_location_pred_non_ml[1]],
    ...
    fig.update_layout(
        title=("LOS" if is_los else "NLOS") + f" (Path delay = {toa:.4f})",
...
def visualize(config: DictConfig) -> None:
    ...
    for batch in datamodule.val_dataloader():
        ...
        for (bs_location, ue_location, toa, aod, aoa, is_los,
            ...
            zip(
                bs_location_, ue_location_, toa_, aod_, aoa_, is_los_,
                image_size_, meta_["path"],
                # ue_location_pred_,
                ue_location_pred_non_ml_, d_pred_non_ml_
            ):
            visualize_pair(
                bs_location, ue_location, toa, aod, aoa, is_los, image_size,
                path,
                # ue_location_pred,
