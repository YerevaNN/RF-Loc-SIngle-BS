...
class TransformerUNet(UNetERM):
    ...
    def pred(self, batch):
        input_image, sequence, mask, supervision_image, ue_location, image_size, is_los = batch
        pred_image = self._network(
            torch.Tensor([input_image]).cuda(self._gpu),
            torch.Tensor([sequence]).cuda(self._gpu),
            torch.Tensor([mask]).cuda(self._gpu)
        )
        return pred_image
    
    def _step(self, batch, *args, **kwargs):
        input_image, sequence, mask, supervision_image, ue_location, image_size, is_los = batch
        pred_image = self._network(input_image, sequence, mask)
